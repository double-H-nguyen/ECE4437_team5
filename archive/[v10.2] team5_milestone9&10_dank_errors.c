 ...
 *
 * CLK: period = 10000 (us)| ANY | Timer Interrupt Every Period | SWI priority = 14
...
void motorTest(void) { //only works if you set period to 10000
    ...
    while(true) {
        ...
        if (!strcmp(command, "1"))    // Stop
        {
            ...
            pwmNow = 5000;
            UARTprintf("speed is reset to %d \n", pwmNow);
        }
...
void distanceSensorTest(void) {
    while (1) {
        ...
        ADCSequenceDataGet(ADC0_BASE, SEQ1, &rightSensorValue); //add to watch expression

        /* Trigger sample sequence 2 */
        ADCProcessorTrigger(ADC0_BASE, SEQ2);

        /* Get results from sample sequence 2 */
        ADCSequenceDataGet(ADC0_BASE, SEQ2, &frontSensorValue); //add to watch expression
